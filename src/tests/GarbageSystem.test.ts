...
describe("garbage pressure", () => {
  it("creates pending garbage when the enemy attack timer elapses", () => {
    ...
    state = new TickCombatUseCase(random).execute(state, 5000);
    ...
    expect(state.combat?.enemy.garbageQueue.getReadyLines()).toBe(0);
  });

  it("inserts garbage and pushes existing rows up when due", () => {
    const random = new SeededRandomProvider(41);
    const state = withPendingGarbage(new StartCombatUseCase(random).execute(new StartRunUseCase().execute()), 1, 0);
  ...
  it("does not insert garbage before travel delay reaches ready", () => {
    const random = new SeededRandomProvider(48);
    const state = withPendingGarbage(new StartCombatUseCase(random).execute(new StartRunUseCase().execute()), 3, 500);
  ...
  it("does not insert pending garbage on a lock that clears lines", () => {
    ...
    const state = withDurableEnemy(withPendingGarbage(new StartCombatUseCase(random).execute(new StartRunUseCase().execute()), 3, 0));
  ...
  it("applies at most four ready garbage lines on a non-clear lock", () => {
    ...
    const state = withPendingGarbage(new StartCombatUseCase(random).execute(new StartRunUseCase().execute()), 10, 0);
  ...
  it("cancels pending garbage before any remaining damage reaches the enemy", () => {
    ...
    const state = withDurableEnemy(withPendingGarbage(new StartCombatUseCase(random).execute(new StartRunUseCase().execute()), 4, 500));
    ...
    combat: {
      ...
      enemy: {
        ...
        definition: {
          ...
          garbagePattern: { type: "fixedInterval", lines: 1, intervalMs: 6000, travelDelayMs: 2500, initialDelayMs: 5000 },
        },
      },
    },
  };
...
function withPendingGarbage(state: GameAppState, pendingGarbage: number, remainingMs: number): GameAppState {
  if (!state.combat) return state;
  return {
    ...state,
    combat: {
      ...
      enemy: {
        ...
        garbageQueue: new GarbageQueue(
          { entryDelayMs: 2500 },
          [{ id: "garbage_1", lines: pendingGarbage, source: "enemy", remainingMs, initialDelayMs: 2500 }],
        ),
        enemyGarbageScheduler: state.combat.enemy.enemyGarbageScheduler ?? new EnemyGarbageScheduler(),
        currentIntent: {
          id: "test_intent",
